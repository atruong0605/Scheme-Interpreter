...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        value_pair = Pair(signature.rest, expressions.rest)
        env.define(signature.first, do_lambda_form(value_pair, env))
...
def do_and_form(expressions, env):
    ...
    and_value = scheme_eval(expressions.first, env)
    rest = expressions.rest
    if is_scheme_false(and_value):
        return and_value
    else:
...
def do_or_form(expressions, env):
    ...
    if not expressions:
        ...
        return scheme_eval(expressions.first, env, True)
    or_value = scheme_eval(expressions.first, env)
    rest = expressions.rest
    if is_scheme_true(or_value):
        return or_value
    else:
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    "*** YOUR CODE HERE ***"
    bindings.map(lambda symbol: validate_form(symbol, 2, 2))
    names = bindings.map(lambda symbol: symbol.first)
    vals = bindings.map(lambda symbol: scheme_eval(symbol.rest.first, env))
...
def do_mu_form(expressions, env):
    ...
    # BEGIN PROBLEM 11
    "*** YOUR CODE HERE ***"
    form = expressions.rest
    return MuProcedure(formals, form)
    # END PROBLEM 11
